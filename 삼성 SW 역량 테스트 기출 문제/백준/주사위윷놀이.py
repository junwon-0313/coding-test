    ...
    def move(org_horse, score, cnt):
        ...
        for i in range(4):
            ...
            if b==0:
                if new_idx>=21:
                    horse[i]=[0,1e4]
                    move(copy.deepcopy(horse),score,cnt+1)
                    continue
                # 끝나는 지점이 바뀌어야한다면 그때 바꿈
                    ...
                    if new_idx==5:
                        ...
                        if [0,new_idx] in horse:
                            continue
                        horse[i]=[0,new_idx]
                    move(copy.deepcopy(horse),score+board[0][new_idx],cnt+1)
                ...
                if new_idx>=8:
                    ...
                    if [3,new_idx] in horse:
                        continue
                    horse[i]=[b,new_idx]
                move(copy.deepcopy(horse),score+board[b][new_idx],cnt+1)
...
#                 yield [array[i]]+next
# print(len(list(combinations([1,2,3,4],10))))
